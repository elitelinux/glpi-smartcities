 ...
 */
class PluginTwinsProfileold extends CommonDBTM
    {
    ...
    function getTabNameForItem(CommonGLPI $item, $withtemplate=0) 
    {
        ...
        elseif (Session::haveRight("profile", "w")){
            if ($item->getType() == 'Profile') { return "Twins";}
        }
       return '';
    }
    ...
    function showForm($id, $options=array()) 
    {
        ...
        $query = "SELECT * FROM glpi_plugin_twins_profiles WHERE id = '$id'";
         if ($result = $DB->query($query)){
            ...
            // Sinon on insère le groupe dans la base avec un droit à null
                ...
                $query = "INSERT INTO glpi_plugin_twins_profiles VALUES ('$id','0')";
    ...
    function majDroit($arrayItem) 
    {
        ...
        $query = "SELECT * FROM glpi_plugin_twins_profiles WHERE id='$arrayItem[0]'";
        if ($result = $DB->query($query)){
            if ($DB->numrows($result) > 0){
                $query = "UPDATE `glpi_plugin_twins_profiles` SET `right`='$arrayItem[1]' WHERE `id`=$arrayItem[0]";
                $DB->query($query);
            }
        }
    }
    ...
    {
        global $DB;
        if (isset($_SESSION["glpiactiveprofile"]["twins"])){
            if($_SESSION["glpiactiveprofile"]["twins"] == "w" || $_SESSION["glpiactiveprofile"]["twins"] == "r")
                {return true;}
        }
        else{
            ...
            if ($result = $DB->query($query)){
                $row = $DB->fetch_assoc($result);
                if (!empty($row['right'])){
                    $_SESSION["glpiactiveprofile"]["twins"] = $row['right'];
                    if($_SESSION["glpiactiveprofile"]["twins"] == "w" || $_SESSION["glpiactiveprofile"]["twins"] == "r")
                        {return true;}
                }
                else{$_SESSION["glpiactiveprofile"]["twins"] = "NULL";}
            }
        }
        return false;
    }

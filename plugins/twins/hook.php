 ...
 */
function plugin_twins_getListeTable()
    {
    $listeTable = Array();

    //==>glpi_plugin_twins_profiles
    $arrayTable = Array("id"=>"int(11) NOT NULL PRIMARY KEY default '0' COMMENT 'RELATION to glpi_profiles (id)'",
            "right"=> "char(1) collate utf8_unicode_ci default NULL");
    $listeTable["glpi_plugin_twins_profiles"] = $arrayTable;
    //==>glpi_plugin_twins_ad
    $arrayTable = Array("id"=>"int(11) NOT NULL PRIMARY KEY AUTO_INCREMENT",
            ...
            "groupe"=> "char(128) NOT NULL default ''",
            "vie"=> "char(1) NOT NULL default '1'");
    $listeTable["glpi_plugin_twins_ad"] = $arrayTable; 
    //==>glpi_plugin_twins_log
    $arrayTable = Array("id"=>"int(11) NOT NULL PRIMARY KEY AUTO_INCREMENT",
            "id_item"=> "int(11) NOT NULL ",
            "type_item"=> "char(32) NOT NULL default ''",
            "technicien"=> "char(32) NOT NULL default ''",
            "date"=> "char(32) NOT NULL default ''",
            "info"=> "char(255) NOT NULL default ''");
    $listeTable["glpi_plugin_twins_log"] = $arrayTable;
    return $listeTable;  
    }
 ...
 */
function plugin_twins_getFirstInsert()
    {
    $id = $_SESSION['glpiactiveprofile']['id'];
    $listeFI[] = "INSERT INTO glpi_plugin_twins_profiles VALUES ('$id','w')";
    return $listeFI;
    }
 ...
 * @return boolean
 */
function plugin_twins_install() 
    {
    global $DB;
    $miseAjour = false;
    //parcour du tableau des tables à créer
    foreach(plugin_twins_getListeTable() as $id => $table)
        {
        if (!TableExists($id)) 
            {
            $requeteInsert = "";
    ...
    if($miseAjour){plugin_twins_miseAjour();}
    else {foreach(plugin_twins_getFirstInsert() as $query){$DB->query($query) or die($DB->error());}} 
    return true;
    }
 ...
 * @return boolean
 */
function plugin_twins_uninstall() 
    {
    global $DB;
    foreach(plugin_twins_getListeTable() as $table => $champ)
 ...
 * @return boolean
 */
function plugin_twins_miseAjour()
    { 
    global $DB;
    $return = true;

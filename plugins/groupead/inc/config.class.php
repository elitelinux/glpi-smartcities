 ...
 */
class PluginGroupeadConfig extends CommonDBTM
    {
    /**
    ...
    function getAD()
        ...
        $query = "SELECT * FROM glpi_plugin_groupead_ad WHERE vie='1'";
    ...
    /**
     * Créé un enregistrement vide dans la base de données pour un nouvel AD
     */
    function setAD()
        {
        global $DB;
        $query = "INSERT INTO glpi_plugin_groupead_ad (serveur,dc,suffix,login,passwd,groupe,vie) VALUES ('','','','','','','1')";
    ...
    function setValeurInfoAD($identifiant,$valeur)
        ...
        if($champ == "valider:")
            ...
            foreach ($valeur as $key => $value)
                ...
                $query = "SELECT $key FROM glpi_plugin_groupead_ad WHERE id!='$id'";
                if ($result = $DB->query($query))
                    {
                    if ($DB->numrows($result) > 0)
                        {
            ...
            if($valid)
                {
                $query = "UPDATE glpi_plugin_groupead_ad SET serveur='".$valeur["serveur"]."',
            ...
            if($champ == "serveur" || $champ == "dc" || $champ == "suffix")
                {
                $query = "SELECT $champ FROM glpi_plugin_groupead_ad WHERE id!='$id'";
                if ($result = $DB->query($query))
                    {
                    if ($DB->numrows($result) > 0)
                        {
            ...
            if($valid)
                ...
                $query = "UPDATE glpi_plugin_groupead_ad SET $champ='$valeur' WHERE id='$id'";
                $DB->query($query);

 ...
 */
class PluginReformeProfile extends Profile{
    
    function getTabNameForItem(CommonGLPI $item, $withtemplate=0) {
        if ($item->getID() > 0 && $item->fields['interface'] == 'central') {
            return self::createTabEntry(__('Reforme', 'reforme'));
        }
    }
    ...
    **/
    ...
    function getAllRights() {
        $a_rights = array(
            ...
            array('rights'    => array(CREATE  => __('Create')),
                'label'     => __('Utilisation reforme', 'reforme'),
                'field'     => 'plugin_reforme_reforme'
            )
        );
        return $a_rights;
    }
    ...
    static function createFirstAccess($profiles_id) {
        include_once(GLPI_ROOT."/plugins/reforme/inc/profile.class.php");
        $profile = new self();

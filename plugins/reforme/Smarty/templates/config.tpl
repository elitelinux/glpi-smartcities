            ...
            <td>
                <table>
                    ...
                    <tr>
                        ...
                        <td width="30px"><button rel=tooltip title="tester le mail" id="btn-modifStructure" onclick="testMail()">{$testIMG}</button>
                        </td>
                    </tr>
                    <tr>
...
<div id="configGestionFinanciere">
    <table class='tab_cadre_fixe'>
        <tr>
            <th colspan="4">Ajout d'informations</th>
        </tr>
        <tr>
            {foreach from=$ajoutInfo item=info key=key}<td>
                    <input type="checkbox" id="info{$key}" name="info{$key}" value="info{$key}" />{$info}
            </td>{/foreach}
        </tr>
        <tr>
            <td colspan="4" align="center"><button rel=tooltip title="Enregistrer" id="btn-validerInfo" onclick="ajouterInfo()">Valider</button></td>
        </tr>
    </table>
</div>
<div id="configAd">
...
<script type="text/javascript">  
    ...
    /**
     * Méthode de chargement de page
     * @returns {undefined}
     */
    $(function() {
        // coche les cases Ajout D'informations
        {/literal}
            {if $infoAdministrative['des_bien']==1 }
                {literal}
                    document.getElementById("info1").checked = true;
                {/literal}
            {/if}
            {if $infoAdministrative['ref_type']==1 }
                {literal}
                    document.getElementById("info2").checked = true;
                {/literal}
            {/if}
            {if $infoAdministrative['num_commande']==1 }
                {literal}
                    document.getElementById("info3").checked = true;
                {/literal}
            {/if}
            {if $infoAdministrative['num_facture']==1 }
                {literal}
    ...
    /**
     * Enregistre les informations supplémentaires à ajouter au bon de réforme
     * @returns {undefined}
     */
    function ajouterInfo()
    {
        var DATA = "action=ajouterInfo&info=";
        {/literal}
            {foreach from=$ajoutInfo item=info key=key}
                {literal}
                    //if(true){
                    if($('#info{/literal}{$key}{literal}').is(':checked')){
                    DATA = DATA + {/literal}{$key}{literal};
                    }{/literal}
            {/foreach}
        {literal}    
   ...
   /**
     * Test la configuration de connexion à un AD
     * @param {type} ad
     * @returns {undefined}
     */
